...
class VideoExportRuntimeTests(VideoExportTestCase):
    def test_run_writes_checks_and_is_idempotent(self):
        ...
        self.assertEqual(
            (destination / VIDEO_OUTPUT).read_bytes(), b"FAKE-MP4"
        )
        self.assertEqual(
    ...
    def test_process_is_shell_free_sanitized_and_isolated(self):
        ...
        original_frame = (
            self.package_dir / "frames/00000000.png"
        ).read_bytes()
        with patch.object(
            video_export_process.subprocess,
            "Popen",
            CapturingPopen,
        ):
            self.execute()
        ...
        self.assertEqual(
            (self.package_dir / "frames/00000000.png").read_bytes(),
            original_frame,
        )
    ...
    def test_existing_staging_conflict_is_preserved(self):
        ...
        self.assertEqual(
            marker.read_text(encoding="utf-8"), "owned elsewhere"
        )

    def test_diagnostic_limit_is_shared_across_stdout_and_stderr(self):
        self.set_fake_program(
            "import os,sys;"
            "os.write(1,b'x'*10);"
            "os.write(2,b'y'*10);"
            "sys.exit(0)"
        ...
        with patch(
            "ai_illustration.video_export_process.MAX_DIAGNOSTIC_BYTES",
            16,
        ):
            with self.assertRaises(VideoExportError) as caught:
                self.execute()
        self.assertEqual(
            caught.exception.code, "FFMPEG_DIAGNOSTIC_LIMIT"
        )
    ...
    def test_executable_cannot_publish_extra_staging_files(self):
        self.set_fake_program(
            ...
            "out.with_name('extra.txt').write_text('extra')"
        )
        with self.assertRaises(VideoExportError) as caught:
            self.execute()
    ...
    def test_failure_cleans_staging(self):
        self.set_fake_program("import sys;sys.exit(7)")
        with self.assertRaises(VideoExportError) as caught:
            self.execute()
    ...
    def test_timeout_and_oversize_failures_clean_staging(self):
        self.set_fake_program("import time;time.sleep(5)")
        ...
        with patch(
            "ai_illustration.video_export_execute.MAX_VIDEO_BYTES", 4
        ):
    ...
    def test_checker_rejects_extra_empty_directory_and_symlink_files(self):
        ...
        self.assertEqual(
            extra_error.exception.code, "FILE_SET_MISMATCH"
        )
        extra.unlink()
        ...
        self.assertEqual(
            directory_error.exception.code, "FILE_SET_MISMATCH"
        )
        empty_dir.rmdir()
        ...
        self.assertEqual(
            symlinked.exception.code, "FILE_SET_MISMATCH"
        )
    ...
    def test_checker_rejects_profile_and_executable_changes(self):
        ...
        self.assertEqual(
            changed.exception.code, "MANIFEST_BINDING_MISMATCH"
        )
        self.profile_path.write_bytes(canonical(self.profile))

        from ai_illustration import video_export_plan

        real_reference = video_export_plan._executable_reference

        def changed_reference(path):
            reference, resolved = real_reference(path)
            return {**reference, "sha256": "0" * 64}, resolved

        with patch.object(
            video_export_plan,
            "_executable_reference",
            side_effect=changed_reference,
        ), patch(
            "ai_illustration.video_export_source.check_frame_preview_package",
            return_value=self._checker_result(),
        ):
        ...
        self.assertEqual(
            executable_changed.exception.code,
