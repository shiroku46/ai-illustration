...
class VideoExportPlanTests(VideoExportTestCase):
    ...
    def test_zero_and_negative_offset_filters(self):
        ...
        self.assertNotIn("adelay", self.plan()["video_export_plan"]["audio_filter"])
        self.manifest["audio_placement"]["offset_ms"] = -250
        self._write_manifest()
        self.assertIn("atrim=start=250ms", self.plan()["video_export_plan"]["audio_filter"])

    def test_rejects_nonopaque_frame(self):
        payload = encode_rgba_png(RGBAImage(2, 2, bytes([255, 0, 0, 0] + [255, 0, 0, 255] * 3)))
        path = self.package_dir / "frames/00000000.png"
        path.write_bytes(payload)
        self.manifest["files"][0] = {"path": "frames/00000000.png", "sha256": sha(payload), "size": len(payload)}
        self._write_manifest()
    ...
    def test_profile_id_and_executable_are_bound(self):
        ...
        self.assertEqual(result["ffmpeg"]["sha256"], sha(self.ffmpeg.read_bytes()))
        broken = dict(self.profile)
        broken["id"] = "paper-theater-video-export-profile-" + "0" * 20
    ...
    def test_module_cli_plan(self):
        argv = [
            "plan", str(self.manifest_path), str(self.profile_path),
            "--ffmpeg", str(self.ffmpeg),
            "--frame-preview-root", str(self.frame_preview_root),
            "--frame-render-root", str(self.frame_render_root),
            "--renderer-job-root", str(self.renderer_root),
            "--render-plan-root", str(self.plan_root),
            "--audio-preview-root", str(self.audio_preview_root),
            "--preview-root", str(self.preview_root),
        ...
        stderr = io.StringIO()
        with patch("ai_illustration.video_export_source.check_frame_preview_package", return_value=self._checker_result()), patch("sys.stdout", output), redirect_stderr(stderr):
            code = main(argv)
            output.flush()
