...
def _png() -> bytes:
    ...
    srgb = bytes([0])
    raw = bytes([0, 20, 40, 60, 255])
    return signature + _chunk(b"IHDR", ihdr) + _chunk(b"sRGB", srgb) + _chunk(b"IDAT", zlib.compress(raw)) + _chunk(b"IEND", b"")
...
class ReviewUITest(unittest.TestCase):
    ...
    def test_missing_image_becomes_metadata_placeholder(self) -> None:
        ...
        self.assertIsNone(data.candidates[0].asset_path)

    def test_verified_png_is_available(self) -> None:
        payload = self.seed(with_image=True)
        data = load_review_data(self.manifests, self.assets)
        self.assertTrue(data.candidates[0].payload["image_available"])
        self.assertEqual(data.candidates[0].asset_path.read_bytes(), payload)
    ...
    def test_review_export_is_existing_schema_valid(self) -> None:
        ...
        self.assertTrue(review["id"].startswith("review-candidate-demo-"))
