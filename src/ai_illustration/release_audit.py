...
SOFTWARE_COMPLETION_DEFINITION = (
    "Software MVP is complete when the release audit, repository validation, and full automated test suite succeed for "
    "all declared quality-stage, art-direction, benchmark, identity-lock, variant-review, asset-preparation, local generation, "
    "packaging, paper-theater, rendering, video-export, and workspace software gates."
)
CONTENT_COMPLETION_DEFINITION = (
    "Project content is complete only after the owner supplies the canonical art references, approved local tools, models and workflows, "
    "executes the real benchmark and identity-lock runs, records human model, identity and variant reviews plus licensing decisions, "
    "supplies source audio, and makes any desired final encoded media and publication decisions."
)

EXPECTED_WORKSPACE_KINDS = frozenset({
    "manifest-set", "comfyui-execution", "variant-set", "export-package",
    "scene-plan", "preview-package", "audio-preview", "render-plan",
    "renderer-job", "frame-render", "frame-preview", "video-export",
})
EXPECTED_MAIN_COMMANDS = frozenset({
    "validate", "catalog-validate", "catalog-list", "catalog-compat",
    "adapter-check", "adapter-plan", "adapter-run", "adapter-run-check",
    "review-ui", "variant-plan", "variant-check", "variant-export",
    "export-check", "paper-plan", "paper-check", "preview-plan",
    "preview-check", "audio-preview-plan", "audio-preview-check",
    "render-plan", "render-plan-check", "composition-job",
    "composition-job-check", "frame-render", "frame-render-check",
})
...
QUALITY_RESET_CRITICAL_PATHS = frozenset({
    "docs/QUALITY_STAGES.md",
    "web/review/index.html",
    "web/review/app.js",
    "web/review/styles.css",
    "schemas/candidate-asset.schema.json",
    "schemas/review-decision.schema.json",
    "src/ai_illustration/quality.py",
    "docs/ART_DIRECTION.md",
    "schemas/art-direction-profile.schema.json",
    "schemas/art-direction-review.schema.json",
    "src/ai_illustration/art_direction.py",
    "docs/MODEL_BENCHMARK.md",
    "docs/BENCHMARK_RESULTS.md",
    "docs/BENCHMARK_REVIEW.md",
    "schemas/model-benchmark-plan.schema.json",
    "schemas/model-benchmark-results.schema.json",
    "schemas/model-benchmark-review.schema.json",
    "src/ai_illustration/model_benchmark.py",
    "src/ai_illustration/benchmark_results.py",
    "src/ai_illustration/benchmark_review.py",
    "docs/IDENTITY_LOCK.md",
    "docs/IDENTITY_LOCK_RESULTS.md",
    "docs/IDENTITY_LOCK_REVIEW.md",
    "schemas/identity-lock-plan.schema.json",
    "schemas/identity-lock-results.schema.json",
    "schemas/identity-lock-review.schema.json",
    "src/ai_illustration/identity_lock.py",
    "src/ai_illustration/identity_lock_results.py",
    "src/ai_illustration/identity_lock_review.py",
    "schemas/variant-set.schema.json",
    "schemas/variant-review-decision.schema.json",
    "schemas/variant-export-package.schema.json",
    "src/ai_illustration/variant_review.py",
    "docs/VARIANT_REVIEW.md",
...
def _validate_contract(value: dict[str, Any], root: Path) -> None:
    expected_fields = {
        "id", "kind", "schema_version", "release_name", "version", "python_requires",
        "runtime_dependencies", "workspace_check_kinds", "main_cli_commands",
        "dedicated_cli_commands", "entry_points", "critical_paths", "protected_sources",
        "external_prerequisites", "human_decisions", "prohibited_effects",
        "software_completion_definition", "content_completion_definition",
    }
    ...
    if (
        value.get("software_completion_definition") != SOFTWARE_COMPLETION_DEFINITION
        or value.get("content_completion_definition") != CONTENT_COMPLETION_DEFINITION
    ):
        raise ReleaseAuditError("COMPLETION_DEFINITION", "completion definition changed", "software_completion_definition")

    if frozenset(_sorted_strings(value.get("workspace_check_kinds"), "workspace_check_kinds")) != EXPECTED_WORKSPACE_KINDS:
        raise ReleaseAuditError("WORKSPACE_KINDS", "workspace check kinds differ from the release surface", "workspace_check_kinds")
    if frozenset(_sorted_strings(value.get("main_cli_commands"), "main_cli_commands")) != EXPECTED_MAIN_COMMANDS:
    ...
    if not MINIMUM_CRITICAL_PATHS.issubset(critical):
        raise ReleaseAuditError(
            "CRITICAL_PATHS",
            f"required critical paths are missing: {sorted(MINIMUM_CRITICAL_PATHS - critical)}",
            "critical_paths",
        )
    if not MINIMUM_PROTECTED_SOURCES.issubset(protected) or not protected.issubset(critical):
        raise ReleaseAuditError("PROTECTED_SOURCES", "protected sources are incomplete or outside critical paths", "protected_sources")
    ...
    if value.get("id") != content_identifier("ai-illustration-mvp-release", core, 20):
...
def _critical_inventory(root: Path, paths: list[str], diagnostics: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for index, relative in enumerate(paths):
        ...
        try:
            payload, size = _bounded_file(_safe_path(relative, root, field), field)
...
def _scan_protected_sources(root: Path, paths: list[str], diagnostics: list[dict[str, str]]) -> None:
    forbidden = ("shell" + "=True", "shell " + "= True", "os." + "system(", "ev" + "al(", "ex" + "ec(")
    for index, relative in enumerate(paths):
        field = f"protected_sources[{index}]"
        try:
            payload, _size = _bounded_file(_safe_path(relative, root, field), field)
            text = payload.decode("utf-8")
        except (ReleaseAuditError, UnicodeError) as exc:
            diagnostics.append(exc.to_dict() if isinstance(exc, ReleaseAuditError) else _diagnostic("PROTECTED_SOURCE_READ", str(exc), field))
            continue
        ...
        for match in URL_RE.finditer(text):
            ...
            host = raw_host if raw_host.startswith("[") else raw_host.split(":", 1)[0]
...
def audit_release(contract_path: Path) -> dict[str, Any]:
    ...
    try:
        pyproject_payload, _ = _bounded_file(root / "pyproject.toml", "pyproject.toml")
        project = tomllib.loads(pyproject_payload.decode("utf-8")).get("project", {})
    except (ReleaseAuditError, UnicodeError, tomllib.TOMLDecodeError) as exc:
    ...
    if frozenset(CHECKERS) != EXPECTED_WORKSPACE_KINDS:
        diagnostics.append(_diagnostic("WORKSPACE_REGISTRY", "workspace checker registry differs from release contract", "CHECKERS"))
    try:
        schema_payload, _ = _bounded_file(root / "schemas/ai-illustration-workspace.schema.json", "workspace schema")
        schema = json.loads(schema_payload.decode("utf-8"))
        schema_kinds = frozenset(schema["$defs"]["check"]["properties"]["kind"]["enum"])
    except (ReleaseAuditError, UnicodeError, json.JSONDecodeError, KeyError, TypeError) as exc:
        diagnostics.append(_diagnostic("WORKSPACE_SCHEMA", str(exc), "workspace schema"))
    else:
        if schema_kinds != EXPECTED_WORKSPACE_KINDS:
            diagnostics.append(_diagnostic("WORKSPACE_SCHEMA", "workspace schema kinds differ from release contract", "workspace schema"))

    if _subcommands(build_parser()) != EXPECTED_MAIN_COMMANDS:
        diagnostics.append(_diagnostic("MAIN_COMMANDS", "main CLI commands differ from release contract", "main CLI"))
    dedicated_actual = {
    ...
    for name, expected in EXPECTED_DEDICATED_COMMANDS.items():
        if dedicated_actual[name] != expected:
            diagnostics.append(_diagnostic("DEDICATED_COMMANDS", f"{name} commands differ from release contract", name))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        f"software MVP audit: {'complete' if result['complete'] else 'incomplete'} ({len(result['diagnostics'])} diagnostic(s))",
        file=sys.stderr,
    )
