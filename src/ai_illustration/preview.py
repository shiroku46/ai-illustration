...
def _js_bytes() -> bytes:
    return b"(()=>{'use strict';const root=document.getElementById('preview');const data=JSON.parse(root.dataset.preview);const stage=document.getElementById('stage');stage.style.setProperty('--canvas-ratio',`${data.width}/${data.height}`);const images={boke:document.getElementById('boke'),tsukkomi:document.getElementById('tsukkomi')};const scrub=document.getElementById('scrub');const current=document.getElementById('current');let playing=false;let started=0;let offset=0;let frame=0;function segmentAt(ms){let result=data.segments[data.segments.length-1];for(const segment of data.segments){if(ms>=segment.start_ms&&ms<segment.end_ms){result=segment;break}}return result}function render(ms){const bounded=Math.max(0,Math.min(data.duration_ms,Math.floor(ms)));const segment=segmentAt(bounded===data.duration_ms?Math.max(0,bounded-1):bounded);for(const role of ['boke','tsukkomi']){const image=images[role];const state=segment[role];image.src=state.asset_path;image.className=`character slot-${segment.stage_slots[role]}`;image.dataset.sha256=state.png_sha256}scrub.value=String(bounded);current.value=String(bounded);offset=bounded}function tick(now){if(!playing)return;const ms=offset+(now-started);render(ms);if(ms>=data.duration_ms){playing=false;return}frame=requestAnimationFrame(tick)}document.getElementById('play').addEventListener('click',()=>{if(playing)return;if(offset>=data.duration_ms)render(0);playing=true;started=performance.now();frame=requestAnimationFrame(tick)});document.getElementById('pause').addEventListener('click',()=>{playing=false;cancelAnimationFrame(frame)});document.getElementById('restart').addEventListener('click',()=>{playing=false;cancelAnimationFrame(frame);render(0)});scrub.addEventListener('input',()=>{playing=false;cancelAnimationFrame(frame);render(Number(scrub.value))});render(0)})();\n"
