...
REQUIRED = {
    ...
    "review-decision": ("kind", "schema_version", "id", "candidate_ref", "decision", "reviewer", "timestamp", "categories"),
    ...
    if kind == "export-manifest":
        ...
        if all(field in data for field in needed):
            try:
                ...
                expected_path, expected_sidecar = export_paths(character_id=character_id, crop=data["crop"], facing=data["facing"], pose=data["pose"], expression=data["expression"], version=data["version"], sha256=data["sha256"])
